...
class NewAuditQueryDialog(QDialog):
    ...
    def _init_ui(self) -> None:
        ...
        self.query_text_input.setPlaceholderText(
            "Detailed query / inquiry regarding ledger entries, missing vouchers, or terms..."
        )
        ...
        self.evidence_requested_input.setPlaceholderText(
            "e.g. Invoices #1021-1035 and Bank Confirmation"
        )
    ...
    def _validate_and_accept(self) -> None:
        if not self.area_input.text().strip() or not self.query_text_input.toPlainText().strip():
            QMessageBox.warning(
                self, "Validation Error", "Please provide Audit Area and Query Text."
            )
            return
        self.accept()
...
class AuditQueryView(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        subtitle = QLabel(
            "Log audit inquiries, track client responses, resolve points, or escalate to formal audit findings."
        )
        ...
        self.card_total = MetricCard(
            "TOTAL QUERIES", "0", "Raised on engagement", accent_color="#2563eb"
        )
        self.card_sent = MetricCard(
            "PENDING CLIENT", "0", "Awaiting response", accent_color="#f59e0b"
        )
        self.card_resolved = MetricCard(
            "RESOLVED", "0", "Satisfactorily closed", accent_color="#10b981"
        )
        self.card_escalated = MetricCard(
            "ESCALATED FINDINGS", "0", "Converted to findings", accent_color="#ef4444"
        )
    ...
    def refresh(self) -> None:
        ...
        escalated_cnt = sum(
            1 for q in queries if q.status == AuditQueryStatusEnum.ESCALATED_TO_FINDING
        )
    ...
    def _make_action_widget(self, q: AuditQuery) -> QWidget:
        ...
        if q.status not in (
            AuditQueryStatusEnum.RESOLVED,
            AuditQueryStatusEnum.ESCALATED_TO_FINDING,
        ):
            ...
            btn_res.setStyleSheet(
                "background-color: #10b981; color: white; font-size: 11px; font-weight: bold; padding: 3px 8px;"
            )
            ...
            btn_esc.setStyleSheet(
                "background-color: #ef4444; color: white; font-size: 11px; font-weight: bold; padding: 3px 8px;"
            )
            btn_esc.clicked.connect(lambda _, query=q: self._on_escalate(query))
            lay.addWidget(btn_esc)
        else:
            lbl_done = QLabel(
                "Closed" if q.status == AuditQueryStatusEnum.RESOLVED else "Escalated"
            )
    ...
    def _on_log_reply(self, query_id: str) -> None:
        text, ok = QInputDialog.getMultiLineText(
            self, "Log Client Response", "Enter explanation or documents provided by client:"
        )
    ...
    def _on_resolve(self, query_id: str) -> None:
        notes, ok = QInputDialog.getText(
            self, "Resolve Audit Query", "Enter resolution notes / audit verification conclusion:"
        )
    ...
    def _on_escalate(self, q: AuditQuery) -> None:
        title, ok1 = QInputDialog.getText(
            ...
            "Escalate to Finding",
            "Audit Finding Title:",
            text=f"Unresolved Query: {q.audit_area}",
        )
        ...
        desc, ok2 = QInputDialog.getMultiLineText(
            ...
            text=f"Client explanation was inadequate or missing for query:\n{q.query_text}",
        )
        ...
        QMessageBox.information(
            self, "Escalated", f"Query escalated to formal Audit Finding '{title.strip()}'."
        )
        self.refresh()
