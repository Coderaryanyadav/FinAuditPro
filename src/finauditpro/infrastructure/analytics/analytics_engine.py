...
class LegacyAnomalyItem:
    def __init__(self, row_index: int, amount: float = 0.0, rationale: str = "", severity: str = "Medium", transaction_id: str | None = None, date: str | None = None, account_name: str | None = None) -> None:
...
def _to_legacy_ledger_entries(records: list[dict[str, Any]]) -> list[LedgerEntry]:
    ...
    for r in records:
        ...
        amt, dr, cr = float(r.get("amount", 0.0)), float(r.get("debit", 0.0)), float(r.get("credit", 0.0))
        if dr == 0.0 and cr == 0.0 and amt > 0: dr = amt
        entries.append(
            LedgerEntry(
...
class DeterministicAnalyticsEngine:
    ...
    def find_duplicates(cls, records: list[dict[str, Any]]) -> Any:
        ...
        items = [LegacyAnomalyItem(row_index=r_no, rationale=f"Duplicate transaction indicator. {e.computed_evidence}", severity=e.severity) for e in res.exceptions for r_no in e.implicated_rows]
        return LegacyAnalyticsResult(items)

    @classmethod
    def find_large_amounts(cls, records: list[dict[str, Any]], threshold: float = 500000.0) -> Any:
        entries = _to_legacy_ledger_entries(records)
        items = [LegacyAnomalyItem(row_index=e.source_row_no, amount=max(e.debit_paise, e.credit_paise) / 100.0, rationale=f"Amount {max(e.debit_paise, e.credit_paise) / 100.0} >= threshold", severity="High") for e in entries if max(e.debit_paise, e.credit_paise) / 100.0 >= threshold]
        return LegacyAnalyticsResult(items)
