...
class AuditQueryModel(Base):
    ...
    working_paper_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("working_papers.id", ondelete="SET NULL"), nullable=True, index=True
    )
    ...
    escalated_finding_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("audit_findings.id", ondelete="SET NULL"), nullable=True
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
...
class ExternalConfirmationModel(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(50), nullable=False, default="Draft Letter", index=True
    )
    ...
    linked_working_paper_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("working_papers.id", ondelete="SET NULL"), nullable=True
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
