...
class ClientModel(Base):
    ...
    entity_type: Mapped[str] = mapped_column(
        String(100), nullable=False, default="Private Limited Company"
    )
    pan: Mapped[str | None] = mapped_column(String(10), nullable=True, index=True)
    gstin: Mapped[str | None] = mapped_column(String(15), nullable=True, index=True)
...
class EngagementModel(Base):
    ...
    documents: Mapped[list["DocumentModel"]] = relationship(
        "DocumentModel", back_populates="engagement", cascade="all, delete-orphan"
    )
    financial_datasets: Mapped[list["FinancialDatasetModel"]] = relationship(
        "FinancialDatasetModel", back_populates="engagement", cascade="all, delete-orphan"
    )
...
class DocumentModel(Base):
    ...
    engagement: Mapped["EngagementModel"] = relationship(
        "EngagementModel", back_populates="documents"
    )
    pages: Mapped[list["DocumentPageModel"]] = relationship(
        "DocumentPageModel", back_populates="document", cascade="all, delete-orphan"
    )
    tables: Mapped[list["ExtractedTableModel"]] = relationship(
        "ExtractedTableModel", back_populates="document", cascade="all, delete-orphan"
    )
    evidence_links: Mapped[list["EvidenceLinkModel"]] = relationship(
        "EvidenceLinkModel", back_populates="document", cascade="all, delete-orphan"
    )
...
class EvidenceLinkModel(Base):
    ...
    document: Mapped["DocumentModel | None"] = relationship(
        "DocumentModel", back_populates="evidence_links"
    )
...
class FinancialDatasetModel(Base):
    ...
    engagement: Mapped["EngagementModel"] = relationship(
        "EngagementModel", back_populates="financial_datasets"
    )
...
class ProcedureRiskLinkModel(Base):
    ...
    risk_id: Mapped[str] = mapped_column(
        String(36), ForeignKey("audit_risks.id", ondelete="CASCADE"), nullable=False, index=True
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), default=utc_now, nullable=False
    )
...
class AuditFindingModel(Base):
    ...
    procedure_id: Mapped[str | None] = mapped_column(
        String(36),
        ForeignKey("audit_procedures.id", ondelete="SET NULL"),
        nullable=True,
        index=True,
    )
    risk_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("audit_risks.id", ondelete="SET NULL"), nullable=True, index=True
    )
    title: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
    description: Mapped[str] = mapped_column(Text, nullable=False)
    category: Mapped[str] = mapped_column(
        String(100), nullable=False, default="Substantive Exception"
    )
...
class AuditEvidenceModel(Base):
    ...
    finding_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("audit_findings.id", ondelete="SET NULL"), nullable=True, index=True
    )
    procedure_id: Mapped[str | None] = mapped_column(
        String(36),
        ForeignKey("audit_procedures.id", ondelete="SET NULL"),
    ...
    dataset_id: Mapped[str | None] = mapped_column(
        String(36), ForeignKey("financial_datasets.id", ondelete="SET NULL"), nullable=True
    )
    row_index: Mapped[int | None] = mapped_column(nullable=True)
...
class LedgerEntryModel(Base):
    ...
    dataset_id: Mapped[str] = mapped_column(
        String(36),
        ForeignKey("financial_datasets.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
    ...
    dataset: Mapped["FinancialDatasetModel"] = relationship(
        "FinancialDatasetModel", back_populates="ledger_entries"
    )
