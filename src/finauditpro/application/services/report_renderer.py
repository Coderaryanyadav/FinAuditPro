...
def _build_sa700_story(story: list[Any], report: Report, data: dict[str, Any], styles: Any) -> None:
    ...
    if findings:
        story.append(Paragraph(
            ...
            f"During the audit period, {len(findings)} reportable audit observations were assessed and tested:", norm
        ))
    else:
...
def _build_caro_story(story: list[Any], data: dict[str, Any], styles: Any) -> None:
    ...
    story.append(Paragraph(
        ...
        f"examined by us in the normal course of audit, and to the best of our knowledge and belief, we state that:", norm
    ))
...
def render_pdf(report: Report, data: dict[str, Any], pdf_path: Path, is_draft: bool) -> None:
    ...
    story.append(Paragraph(f"<b>Data As-Of:</b> {data.get('as_of', '')[:19]} | <b>Generated By:</b> {report.generated_by}", styles["Normal"]))
    story.append(Spacer(1, 10))
    ...
    story.append(table)
    doc.build(story, canvasmaker=lambda *args, **kwargs: WatermarkedCanvas(*args, is_draft=is_draft, **kwargs))
