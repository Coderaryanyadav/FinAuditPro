...
class DeterministicAnalyticsEngine:
    ...
    def check_trial_balance_balances(dataset_id: str, lines: list[TrialBalanceLine]) -> AnalyticRunResult:
        ...
        exceptions: list[ExceptionItem] = []
        if imbalance > 0:
            dr_m = Money(paise=total_dr)
            cr_m = Money(paise=total_cr)
            diff_m = Money(paise=imbalance)
            exceptions.append(
                ExceptionItem(
                    analysis_run_id="",
    ...
    def check_gl_tb_agreement(
        ...
        for entry in gl_entries:
            ...
            net = entry.debit_paise - entry.credit_paise
            gl_movements[key] = gl_movements.get(key, 0) + net

        exceptions: list[ExceptionItem] = []
        for line in tb_lines:
            key = (line.account_code or line.account_name or "").strip().lower()
            tb_net = line.debit_paise - line.credit_paise
            gl_net = gl_movements.get(key, 0)
            if tb_net != gl_net:
                diff = abs(tb_net - gl_net)
                exceptions.append(
                    ExceptionItem(
                        analysis_run_id="", dataset_id=dataset_id, analytic_id="gl_tb_agreement", severity="High",
        ...
        return AnalyticRunResult(
            analytic_id="gl_tb_agreement", analytic_version="1.0.0", title="General Ledger to Trial Balance Reconciliation",
            parameters={}, summary=f"Checked {len(tb_lines)} trial balance accounts against general ledger movements. {len(exceptions)} discrepancies found.",
    ...
    def detect_duplicates(dataset_id: str, entries: list[LedgerEntry]) -> AnalyticRunResult:
        ...
        for e in entries:
            dt = e.entry_date or ""
            key = (dt, e.debit_paise, e.credit_paise, (e.account_code or e.account_name or "").strip().lower())
        ...
        for key, dupes in grouped.items():
            if len(dupes) > 1 and (key[1] > 0 or key[2] > 0):
                ...
                exceptions.append(
                    ExceptionItem(
                        analysis_run_id="", dataset_id=dataset_id, analytic_id="duplicate_detection",
        ...
        return AnalyticRunResult(
            analytic_id="duplicate_detection", analytic_version="1.0.0", title="Duplicate Transaction Detection",
    ...
    # 5. Large Amount Outliers (Z-Score)
    @staticmethod
    def detect_large_amount_outliers(
        ...
        if not amounts or len(amounts) < 5:
            return AnalyticRunResult(
                analytic_id="large_amount_outliers", analytic_version="1.0.0", title="Statistical Large-Amount Outlier Indicator",
                parameters={"z_threshold": z_threshold}, summary="Insufficient non-zero entries for statistical outlier analysis.",
                explanation="Calculated z-score: z = (x - mean) / std_dev. Flagged entries with z >= z_threshold.", exceptions=[],
            )
        ...
        for e in entries:
            ...
            if amt > 0:
                ...
                if z_score >= z_threshold:
                    exceptions.append(
                        ExceptionItem(
                            analysis_run_id="", dataset_id=dataset_id, analytic_id="large_amount_outliers",
        ...
        return AnalyticRunResult(
            ...
            parameters={"z_threshold": z_threshold, "mean_inr": round(mean_val / 100.0, 2), "std_dev_inr": round(std_dev / 100.0, 2)},
    ...
    def detect_round_number_amounts(
        dataset_id: str, entries: list[LedgerEntry], min_paise: int = 10000000  # ₹1,00,000
    ) -> AnalyticRunResult:
        exceptions: list[ExceptionItem] = []
        modulus_paise = 1000000  # ₹10,000 in paise
        ...
        return AnalyticRunResult(
            analytic_id="round_number_amounts", analytic_version="1.0.0", title="Round-Number Transaction Inspection",
            ...
            explanation="Filtered entries where amount >= min_amount and (amount % 10000 == 0).", exceptions=exceptions,
        )
    ...
    def detect_weekend_postings(dataset_id: str, entries: list[LedgerEntry]) -> AnalyticRunResult:
            ...
            if e.entry_date:
                try:
                    ...
                    if dt.weekday() in (5, 6):
                        ...
                        exceptions.append(
                            ExceptionItem(
                                analysis_run_id="", dataset_id=dataset_id, analytic_id="weekend_postings",
                                ...
                                implicated_rows=[e.source_row_no], computed_evidence=f"Date: '{e.entry_date}' is {day_name} (weekday={dt.weekday()}).",
                            )
                        )
                except ValueError:
                    pass
        ...
        return AnalyticRunResult(
            analytic_id="weekend_postings", analytic_version="1.0.0", title="Weekend Transaction Posting Indicator",
            parameters={"non_business_days": "Saturday, Sunday"}, summary=f"Flagged {len(exceptions)} transactions dated on weekends.",
            explanation="Evaluated calendar weekday of entry_date. Flagged entries falling on Saturday (5) or Sunday (6).", exceptions=exceptions,
        )

    # 8. Sequential Gap Detection
    @staticmethod
    def detect_sequence_gaps(dataset_id: str, entries: list[LedgerEntry]) -> AnalyticRunResult:
        parsed_nums: list[tuple[int, LedgerEntry]] = []
        for e in entries:
            num_str = e.voucher_number or e.reference or ""
        ...
        for i in range(len(parsed_nums) - 1):
            ...
            if next_num - curr_num > 1:
                ...
                exceptions.append(
                    ExceptionItem(
                        analysis_run_id="", dataset_id=dataset_id, analytic_id="sequential_gap_detection", severity="Medium",
                        ...
                        implicated_rows=[curr_e.source_row_no], computed_evidence=f"Voucher #{curr_num} followed by #{next_num} (Missing range: {curr_num + 1} to {next_num - 1}).",
                    )
                )

        return AnalyticRunResult(
            analytic_id="sequential_gap_detection", analytic_version="1.0.0", title="Numerical Voucher / Invoice Sequence Gap Detection",
            parameters={}, summary=f"Detected {len(exceptions)} numerical sequence gaps in voucher numbers.",
            explanation="Extracted numeric digits from voucher numbers, sorted sequentially, and identified gaps where next_num - curr_num > 1.", exceptions=exceptions,
        )

    # 9. Benford's Law First-Digit Deviation
    @staticmethod
    def check_benford_law(dataset_id: str, entries: list[LedgerEntry]) -> AnalyticRunResult:
        ...
        if len(first_digits) < 50:
            return AnalyticRunResult(
                analytic_id="benford_law_deviation", analytic_version="1.0.0", title="Benford's Law First-Digit Distribution Inspection",
                parameters={}, summary="Sample size too small (<50 entries) for Benford's Law statistical analysis.",
                explanation="Requires at least 50 non-zero monetary values to perform first-digit distribution check.", exceptions=[],
            )
        ...
        if chi_square > 15.51:
            exceptions.append(
                ExceptionItem(
                    analysis_run_id="", dataset_id=dataset_id, analytic_id="benford_law_deviation", severity="Medium",
                    ...
                    implicated_rows=[], computed_evidence=f"Observed vs Expected First-Digit %: Digit 1 ({observed_freq[1]:.1%} vs {benford_expected[1]:.1%}), Digit 9 ({observed_freq[9]:.1%} vs {benford_expected[9]:.1%}).",
                )
            )
        ...
        return AnalyticRunResult(
            ...
            explanation="Calculated first-digit empirical distribution (digits 1-9) vs Benford's Law curve log10(1 + 1/d). Evaluated Chi-Square goodness-of-fit.", exceptions=exceptions,
        )

    # Legacy Compatibility Wrappers for Dict Records
    @classmethod
