...
class DocumentViewerDialog(QDialog):
    ...
    def _init_ui(self) -> None:
        ...
        title.setStyleSheet("font-size: 16px; font-weight: 700; color: #2563EB;")
        size_mb = doc.file_size_bytes / (1024 * 1024)
        hash_snip = f"{doc.content_hash[:16]}..."
        status_color = "#16A34A" if doc.status.value in ("Ready", "Completed") else "#DC2626"
    ...
    def _refresh_evidence_table(self) -> None:
        ...
        self.evidence_table.setRowCount(len(links))
...
class LinkEvidenceDialog(QDialog):
    ...
    def __init__(self, document_name: str, page_number: int, selected_snippet: str = "", parent: QWidget | None = None) -> None:
        ...
        info_lbl.setStyleSheet("font-size: 13px; font-weight: 700; color: #2563EB;")
        layout.addWidget(info_lbl)

        form = QFormLayout()
        self.title_input = QLineEdit(f"Page {page_number} Evidence")
        self.target_type_input = QLineEdit("Audit Finding")
        self.snippet_input = QLineEdit(selected_snippet)
