...
def check_python_version():
    ...
    if major < 3 or (major == 3 and minor < 10):
        ...
        sys.exit(1)
    print(f"[OK] Python {major}.{minor} meets system requirements.\n")
...
def check_virtual_environment():
    ...
    if in_venv:
        print(f"[OK] Active virtual environment detected: {sys.prefix}\n")
        return sys.executable
    elif venv_dir.exists():
        print(f"[OK] Existing .venv directory found at {venv_dir}")
        ...
        try:
            ...
            print(f"[WARN] Could not create venv automatically: {e}. Proceeding with global python.\n")
            return sys.executable
...
def install_python_dependencies(python_bin):
    ...
    try:
        ...
        print(f"[WARN] Package installation encountered warnings/errors: {e}\n")

def check_system_tools():
    print("[4/5] Checking System Dependencies (Tesseract OCR & Ollama)...")
    
    # Check Tesseract
    tesseract_path = shutil.which("tesseract")
    if tesseract_path:
        print(f"  [OK] Tesseract OCR found at: {tesseract_path}")
    else:
        print("  [WARN] Tesseract OCR not detected in PATH.")
    ...
    if ollama_path:
        print(f"  [OK] Ollama Local AI daemon found at: {ollama_path}")
    else:
        print("  [INFO] Ollama AI daemon optional for local LLM features.")
...
def launch_application(python_bin):
    ...
    if not main_py.exists():
        print(f"[ERROR] Main application file not found at {main_py}")
        return
    ...
    try:
        ...
        print("\n[INFO] FinAuditPro shut down cleanly.")
