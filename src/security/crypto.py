...
class AESCryptoEngine:
    ...
    def __init__(self, master_password: str = "FinAuditPro-Master-Key-2026"):
        # Derive 32-byte key from master password via SHA-256
        self.key = hashlib.sha256(master_password.encode("utf-8")).digest()
        self._fernet = None
        self._init_fernet()
    ...
    def _init_fernet(self):
        try:
            ...
            logger.warning("cryptography package not installed. Using internal XOR stream cipher fallback.")

    def encrypt_bytes(self, data: bytes) -> bytes:
        """Encrypt byte array using AES-256 (Fernet) or fallback cipher."""
        if self._fernet:
