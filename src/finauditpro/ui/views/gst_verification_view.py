...
class GSTVerificationView(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        stats = QHBoxLayout()
        stats.setSpacing(10)
        self.card_total = MetricCard("TOTAL INVOICES", "0", "Purchase register entries", accent_color="#0284C7")
        self.card_matched = MetricCard("MATCHED IN 2B", "0", "Full credit available", accent_color="#16A34A")
        self.card_mismatch = MetricCard("MISMATCHED / MISSING", "0", "Action required", accent_color="#DC2626")
        self.card_ineligible = MetricCard("INELIGIBLE SEC 17(5)", "0", "Blocked ITC claims", accent_color="#D97706")
        ...
        self.table.setHorizontalHeaderLabels([
            "INVOICE NO", "DATE", "VENDOR NAME", "GSTIN",
            "TAXABLE AMT (₹)", "BOOKS ITC (₹)", "2B ITC (₹)", "VARIANCE (₹)", "RECON STATUS"
        ])
    ...
    def load_data(self) -> None:
        ...
        for idx, row in enumerate(self._invoices):
            ...
            status_str = row[8]
            if "matched" in status_str.lower(): matched_cnt += 1
            elif "ineligible" in status_str.lower(): ineligible_cnt += 1
            else: mismatch_cnt += 1
