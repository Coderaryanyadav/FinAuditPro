...
class DashboardView(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        for i, (name, icon, is_done, is_active) in enumerate(self.steps_data):
            ...
            slbl.setStyleSheet(
                f"background-color: {bg}; color: {fg}; font-size: 11px; font-weight: {fw}; border: 1px solid {bd}; border-radius: 4px; padding: 5px 2px;"
            )
        ...
        act_hdr.setStyleSheet(
            "font-size: 10px; font-weight: 700; color: #64748B; border: none; background: transparent;"
        )
        self.act_title = QLabel("Register your first audit client")
        self.act_title.setStyleSheet(
            "font-size: 16px; font-weight: 700; color: #0F172A; border: none; background: transparent;"
        )
        self.act_desc = QLabel(
            "Add a client entity to begin statutory audit planning, materiality assessment, and ledger scrutiny."
        )
        self.act_desc.setStyleSheet(
        ...
        self.btn_go.setStyleSheet(
            "QPushButton { background-color: #2563EB; color: #FFFFFF; font-size: 12px; font-weight: 600; border-radius: 6px; padding: 7px 14px; border: 1px solid transparent; } QPushButton:hover { background-color: #1D4ED8; }"
        )
        ...
        self.att_txt.setStyleSheet(
            "font-size: 13px; font-weight: 500; color: #334155; border: none; background: transparent;"
        )
        ...
        self.table_projects.setVisible(False)
    ...
    def refresh_dashboard(self) -> None:
        clients = (
            self.client_service.list_clients_for_firm(self.current_firm.id)
        ...
        completed_cnt = sum(
            ...
            if str(getattr(e, "status", "")).lower() in ("completed", "signed off", "locked")
        )
        ...
        if has_risk_exposure:
            ...
            self.lbl_zero_risk.setStyleSheet(
                "font-size: 12px; font-weight: 600; color: #DC2626; padding: 10px; border: none; background: transparent;"
            )
        else:
        ...
        if not clients:
            ...
            self.act_desc.setText(
                "Set up the financial year, audit scope, and statutory terms to activate audit tools."
            )
            ...
            if not mat:
                ...
                self.act_desc.setText(
                    "Calculate overall & performance materiality (SA 320) to establish testing thresholds."
                )
                ...
                self.act_desc.setText(
                    "All procedures concluded, reports generated, and working papers sealed."
                )
                ...
                self.act_desc.setText(
                    "Review risks, execute substantive test procedures, and inspect working papers."
                )
        ...
        for idx, eng in enumerate(all_engagements[:10]):
            ...
            if any(
                getattr(r.inherent_risk, "value", str(r.inherent_risk)).lower() == "high"
                for r in eng_risks
            ):
                risk_lbl = "● High"
            elif any(
                getattr(r.inherent_risk, "value", str(r.inherent_risk)).lower() == "medium"
                for r in eng_risks
            ):
                risk_lbl = "● Medium"
            else:
