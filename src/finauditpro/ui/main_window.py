...
NAV_ITEMS = [
    ...
    ("btn_audit_matrix", "① Planning & SA 320", "GUIDED PIPELINE"), ("btn_financial_data", "② TB/GL & Analytics", "GUIDED PIPELINE"),
    ("btn_working_papers", "③ Working Papers", "GUIDED PIPELINE"), ("btn_reports", "④ Reports & Sign-Off", "GUIDED PIPELINE"),
    ("btn_documents", "Document Evidence", "SUPPORTING TOOLS"), ("btn_gst", "GST Reconciliation", "SUPPORTING TOOLS"),
    ("btn_compliance", "Statutory Compliance", "SUPPORTING TOOLS"), ("btn_ai_assistant", "AI Copilot Lab", "ANALYSIS"),
    ("btn_archival", "Engagement Archival", "SYSTEM"), ("btn_roll_forward", "Roll-Forward Tie-Out", "SYSTEM"),
...
class MainWindow(QMainWindow):
    """Main Application Shell Window for FinAuditPro Tier-1 Guided Audit OS."""
    ...
    def _init_ui(self) -> None:
        ...
        logo_box = _tag(QLabel("FA"), "sidebarLogoBadge")
        ...
        self.btn_collapse.setStyleSheet("QPushButton { border: none; background: transparent; color: #64748B; font-size: 10px; font-weight: 700; }")
        ...
        u_info = QVBoxLayout()
        u_info.setSpacing(0)
        u_info.addWidget(_tag(QLabel("Partner"), "userName"))
        u_info.addWidget(_tag(QLabel("Chartered Accountant"), "userRole"))
        btn_more = QPushButton("•••")
        btn_more.setFixedSize(22, 22)
        btn_more.setStyleSheet("QPushButton { border: none; background: transparent; color: #64748B; font-weight: 800; }")
        ...
        sf_s.setPlaceholderText("Search procedures, clients, findings...")
        ...
        self.eng_selector_combo = _tag(QComboBox(), "clientSelectorCombo")
        ...
        self.btn_copilot_toggle.setStyleSheet("QPushButton { background: #1e293b; color: #38bdf8; border: 1px solid #0284c7; border-radius: 6px; padding: 6px 14px; font-weight: 700; font-size: 12px; } QPushButton:hover { background: #0369a1; color: #ffffff; }")
        ...
        # 3. Stacked Views & AI Drawer in Split Layout
        ...
        QShortcut(QKeySequence("Ctrl+K"), self, self._toggle_ai_drawer)
        QShortcut(QKeySequence("Meta+K"), self, self._toggle_ai_drawer)
    ...
    def _init_views(self) -> None:
        ...
        for v in (self.view_dashboard, self.view_firms, self.view_clients, self.view_engagements, self.view_audit_matrix, self.view_financial_data, self.view_working_papers, self.view_reports, self.view_documents, self.view_gst, self.view_compliance, self.view_ai_assistant, self.view_archival, self.view_roll_forward, self.view_settings):
    ...
    def _show_profile_menu(self) -> None:
        menu = QMenu(self)
        menu.setStyleSheet("QMenu { background-color: #FFFFFF; border: 1px solid #E2E8F0; padding: 4px; font-size: 12px; }")
        menu.addAction("Firm Settings", lambda: self.btn_settings.click())
        menu.addSeparator()
    ...
    def _auto_select_initial_engagement(self) -> None:
        try:
            ...
            self.set_active_firm(firms[0].id) if firms else self._update_header_combo()
    ...
    def _sync_views_engagement(self, eng: Any) -> None:
        for v in (self.view_documents, self.view_financial_data, self.view_gst, self.view_compliance, self.view_audit_matrix, self.view_working_papers, self.view_reports, self.view_ai_assistant, self.view_archival, self.view_roll_forward):
            getattr(v, "set_active_engagement", lambda _: None)(eng)
