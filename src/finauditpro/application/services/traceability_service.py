...
class TraceabilityService:
    ...
    def build_finding_traceability(
        self, engagement_id: str, finding_id: str
        ...
        with self.db_manager.session_scope() as session:
            ...
            for ev in evidences:
                ev_type = (
                    "DocumentPage"
                    if (ev.document_id or ev.page_number is not None)
                    else "FinancialRow"
                )
            ...
            # 3. Linked Procedure
            proc = (
                matrix_repo.get_procedure_by_id(finding.procedure_id)
                if finding.procedure_id
                else None
            )
            if proc:
