...
def legacy_find_large_amounts(
    cls: Any, records: list[dict[str, Any]], threshold: float = 500000.0
) -> Any:
    entries = to_legacy_ledger_entries(records)
    items = []
    for e in entries:
        ...
        if amt >= threshold:
            items.append(
                LegacyAnomalyItem(
                    ...
                    amount=amt,
                    rationale=f"Amount {amt} >= threshold",
                    severity="High",
                )
            )
    return LegacyAnalyticsResult(items)
...
def legacy_find_round_numbers(
    cls: Any, records: list[dict[str, Any]], min_amount: float = 100000.0
) -> Any:
    entries = to_legacy_ledger_entries(records)
    ...
    for e in res.exceptions:
        ...
        items.append(
            LegacyAnomalyItem(
                row_index=r_no, amount=amt_val, rationale=e.computed_evidence, severity=e.severity
            )
        )
    return LegacyAnalyticsResult(items)
