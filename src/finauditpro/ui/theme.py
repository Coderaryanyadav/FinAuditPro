...
def format_inr(val: Any) -> str:
    ...
    try:
        ...
        if isinstance(val, str):
            val = float(val.replace("₹", "").replace(",", "").strip() or 0)
        neg = val < 0
...
class LightColors:
    BG_BASE, BG_SURFACE, BG_ELEVATED, BG_SUBTLE, BG_HOVER = (
        "#F8FAFC",
        "#FFFFFF",
        "#FFFFFF",
        "#F1F5F9",
    ...
    ACCENT, ACCENT_HOVER, ACCENT_SUBTLE, ACCENT_BORDER = (
        "#2563EB",
        "#1D4ED8",
        "rgba(37, 99, 235, 0.1)",
        "rgba(37, 99, 235, 0.25)",
    ...
    NAV_BG, NAV_BORDER, NAV_TEXT, NAV_TEXT_ACTIVE, NAV_ACTIVE_BG = (
        "#FFFFFF",
        "#E2E8F0",
        "#475569",
        "#2563EB",
...
class CardWidget(QFrame):
    def __init__(self, title: str | None = None, parent: QWidget | None = None) -> None:
        ...
        self.setStyleSheet(
            "QFrame#contentCard { background-color: #FFFFFF; border: 1px solid #E2E8F0; border-radius: 8px; }"
        )
        ...
        if title:
            ...
            h_lbl = QLabel(title)
            h_lbl.setStyleSheet(
                "font-size: 11px; font-weight: 700; color: #64748B; border: none; background: transparent;"
            )
...
class MetricCard(QFrame):
    ...
    def __init__(
        ...
        self.setStyleSheet(
            f"QFrame#metricCard {{ background-color: #FFFFFF; border: 1px solid #E2E8F0; border-radius: 8px; border-top: 2px solid {accent_color}; }}"
        )
        if action_text:
        ...
        self.title_lbl.setStyleSheet(
            "font-size: 11px; font-weight: 600; color: #64748B; border: none; background: transparent;"
        )
        self.value_lbl = QLabel(value)
        self.value_lbl.setStyleSheet(
            "font-size: 24px; font-weight: 600; color: #0F172A; border: none; background: transparent;"
        )
        sub_row = QHBoxLayout()
        ...
        if action_text:
            ...
            self.act_lbl.setStyleSheet(
                "font-size: 11px; font-weight: 600; color: #2563EB; border: none; background: transparent;"
            )
...
class StatusBadge(QLabel):
    ...
    def _update_style(self, status_type: str) -> None:
        ...
        self.setStyleSheet(
            f"font-size: 11px; font-weight: 500; border-radius: 4px; padding: 2px 8px; {style}"
        )
...
class RiskBadge(QLabel):
    def __init__(self, risk_level: str, parent: QWidget | None = None) -> None:
        ...
        self.setStyleSheet(
            f"font-size: 11px; font-weight: 500; border-radius: 4px; padding: 2px 8px; {st}"
        )
...
class EmptyStateWidget(QFrame):
    def __init__(
        ...
        self.setStyleSheet(
            "QFrame#emptyStateWidget { background-color: #FAFBFC; border: 1.5px dashed #CBD5E1; border-radius: 8px; }"
        )
        layout = QVBoxLayout(self)
        ...
        t_lbl.setStyleSheet(
            "font-size: 17px; font-weight: 700; color: #0F172A; border: none; background: transparent; padding-bottom: 2px;"
        )
        ...
        d_lbl.setStyleSheet(
            "font-size: 14px; color: #64748B; border: none; background: transparent; padding-bottom: 4px;"
        )
        ...
        if action_text and action_callback:
            ...
            btn.setStyleSheet(
                "QPushButton { background-color: #2563EB; color: #FFFFFF; font-size: 13px; font-weight: 600; border-radius: 6px; padding: 8px 20px; border: 1px solid transparent; } QPushButton:hover { background-color: #1D4ED8; }"
            )
...
class LoadingStateWidget(QFrame):
    def __init__(
        self, message: str = "Loading audit records...", parent: QWidget | None = None
    ) -> None:
        super().__init__(parent)
        layout = QVBoxLayout(self)
        layout.setContentsMargins(24, 32, 24, 32)
...
class ErrorStateWidget(QFrame):
    def __init__(
        ...
        message: str = "Local audit database could not be queried. No changes were made.",
        retry_callback: object = None,
        parent: QWidget | None = None,
    ) -> None:
        super().__init__(parent)
        layout = QVBoxLayout(self)
        ...
        if retry_callback:
            ...
            btn.setStyleSheet(
                "background-color: #DC2626; color: #FFFFFF; font-size: 13px; font-weight: 500; border-radius: 6px; padding: 6px 14px;"
            )
...
class PageHeader(QFrame):
    def __init__(
        ...
        self.title_lbl = QLabel(title)
        self.title_lbl.setStyleSheet(
            "font-size: 22px; font-weight: 600; color: #0F172A; border: none; background: transparent;"
        )
        ...
        if action_text and action_callback:
            ...
            self.action_btn.setStyleSheet(
                "QPushButton { background-color: #2563EB; color: #FFFFFF; font-size: 13px; font-weight: 500; border-radius: 6px; padding: 7px 16px; border: 1px solid transparent; } QPushButton:hover { background-color: #1D4ED8; }"
            )
