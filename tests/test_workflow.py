...
class TestWorkflowEngine(unittest.TestCase):
    ...
    def test_initial_state(self):
        wm = WorkflowManager()
        state = wm.initialize_engagement(engagement_id=1, client_id=10, financial_year="2025-26")
        self.assertIsNotNone(wm.current_state)
        self.assertIsNotNone(wm.current_state.current_stage)
        self.assertTrue(isinstance(wm.current_state.completion_percentage, float))

    def test_valid_stage_transitions(self):
        wm = WorkflowManager()
        state = wm.initialize_engagement(engagement_id=2, client_id=11, financial_year="2025-26")
        stage = wm.current_state.current_stage
        self.assertIsNotNone(stage)
        self.assertTrue(wm.current_state.completion_percentage >= 0.0)

    def test_workflow_validator(self):
        ws = WorkflowState(engagement_id=1, client_id=10, financial_year="2025-26", current_stage=AuditStage.CLIENT_CREATED)
        valid = WorkflowValidator.validate_transition(
            state=ws,
